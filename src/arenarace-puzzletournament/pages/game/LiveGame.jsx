  ...
  useEffect(() => {
    ...
    newSocket.on('game_data', (data) => {
      ...
      if (startedGameRef.current !== gameId) {
        ...
        trackEvent('game_started', { gameId, timeControl: data.timeControl || null, mode: 'tournament' });
      }
    ...
    newSocket.on('game_finished', ({ result, winner }) => {
      ...
      setWinner(winner);
      trackEvent('game_finished', { result, winner, mode: 'tournament', durationMs: gameStartTimeRef.current ? Date.now() - gameStartTimeRef.current : 0 });
