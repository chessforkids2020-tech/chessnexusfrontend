...
const STATUSES = [
  ...
  { id: 'finished',  label: 'Finished',  color: '#6ee7b7' },
...
export default function AdminFreeClassRequests() {
  ...
  const [counts, setCounts] = useState({ pending: 0, contacted: 0, scheduled: 0, finished: 0, declined: 0 });
  const [filter, setFilter] = useState('');
  const [loading, setLoading] = useState(true);
  const [busyId, setBusyId] = useState(null);
  const [noteDraft, setNoteDraft] = useState({});

  // Total across every status — the "All requests" card.
  ...
  const purge = async () => {
    try {
      const [s, d] = await Promise.all([
        api.post('/api/free-class/admin/requests/purge', { mode: 'finished', days: 30, dryRun: true }),
      ...
      if (!window.confirm(
        ...
        `• ${s.data?.wouldDelete || 0} finished more than 30 days ago\n` +
      ...
      await Promise.all([
        api.post('/api/free-class/admin/requests/purge', { mode: 'finished', days: 30 }),
  ...
  return (
    <div style={S.page}>
      ...
      {/* Status cards: the number is the point, so it leads. Clicking a card
          filters the list below to that status; the active one is outlined. */}
      <div style={S.cards}>
        <button
          style={{ ...S.card, ...(filter === '' ? S.cardOn : {}) }}
          onClick={() => setFilter('')}
        >
          <span style={{ ...S.cardNum, color: '#e6e8ee' }}>{totalCount}</span>
          <span style={S.cardLabel}>All requests</span>
        </button>
        {STATUSES.map(st => (
          <button
            key={st.id}
            style={{ ...S.card, ...(filter === st.id ? S.cardOn : {}) }}
            onClick={() => setFilter(st.id)}
          >
            <span style={{ ...S.cardNum, color: st.color }}>{counts[st.id] ?? 0}</span>
      ...
      <div style={S.filters}>
        ...
        {filter && (
          <button style={S.ghost} onClick={() => setFilter('')}>✕ Clear filter</button>
        )}
      </div>
...
const S = {
  // Full-bleed dark. The App-level wrapper (beige #f5f5dc) is deliberately not
  // used for this route, so the background has to come from here or the page
  // sits in a cream frame.
  page: {
    minHeight: '100vh', width: '100%', background: '#05060a', color: '#e6e8ee',
    boxSizing: 'border-box',
  },
  inner: { maxWidth: 1100, margin: '0 auto', padding: '26px 20px 80px' },
  ...
  cardOn: { borderColor: 'rgba(52,211,153,0.5)', background: 'rgba(16,185,129,0.10)' },
  cardNum: { display: 'block', fontSize: 26, fontWeight: 900, lineHeight: 1.15 },
  cardLabel: { display: 'block', marginTop: 2, fontSize: 11, fontWeight: 700, letterSpacing: '.5px', color: '#94a3b8', textTransform: 'uppercase' },
