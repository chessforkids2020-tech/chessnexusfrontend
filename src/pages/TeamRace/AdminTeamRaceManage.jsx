...
function AdminTeamRaceManage() {
  ...
  useEffect(() => {
    ...
    // Join race room for live updates and request authoritative time
    socket.emit('joinTeamRace', { raceId });
    socket.emit('requestTimeSync', { raceId });

    const onTeamRaceTime = (data) => {
      if (!data) return;
      if (data.raceId && String(data.raceId) !== String(raceId)) return;
      const serverTime = data.serverTime ? new Date(data.serverTime).getTime() : Date.now();
      const offset = serverTime - Date.now();
      setServerOffsetMs(offset);
      setServerNowMs(Date.now() + offset);
      if (data.startTime || data.status) {
        setRace(prev => ({ ...(prev || {}), startTime: data.startTime || prev?.startTime, status: data.status || prev?.status }));
      }
    };
    socket.on('teamRaceTime', onTeamRaceTime);
    ...
    return () => {
      ...
      socket.off('teamRaceTime', onTeamRaceTime);
  ...
  const formatCountdown = (target) => {
    ...
    const now = serverOffsetMs ? serverNowMs : Date.now();
    const diff = t - now;
  ...
  // keep serverNowMs ticking
  useEffect(() => {
    const t = setInterval(() => {
      if (serverOffsetMs) setServerNowMs(Date.now() + serverOffsetMs);
    }, 1000);
