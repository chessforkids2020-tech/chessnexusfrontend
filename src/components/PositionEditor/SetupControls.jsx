...
const sectionLabel = { fontSize: 12, color: '#9ca3af', marginBottom: 8, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1 };

// Shared helper to overwrite one FEN field and keep the clocks valid.
function setFenPart(chess, onFenChange, index, value) {
...
// ── Side to Move (standalone) ──
export function SideToMoveControl({ chess, onFenChange }) {
  const turn = chess.fen().split(' ')[1] || 'w';
  return (
    <div>
      <div style={sectionLabel}>Side to Move</div>
      <div style={{ display: 'flex', gap: 8 }}>
        {['w', 'b'].map(c => (
          <button
            key={c}
            onClick={() => setFenPart(chess, onFenChange, 1, c)}
            style={{
              ...
              borderRadius: 8, color: turn === c ? '#fff' : '#6b7280', cursor: 'pointer', fontSize: 13, fontWeight: 700, transition: 'all 0.15s',
            }}
          >
            {c === 'w' ? '♔ White' : '♚ Black'}
          </button>
...
// ── Castling Rights (standalone) ──
export function CastlingControl({ chess, onFenChange }) {
  const castling = chess.fen().split(' ')[2] || '-';
  function toggleCastle(flag) {
    let c = castling === '-' ? '' : castling;
    if (c.includes(flag)) c = c.replace(flag, '');
    else { const order = 'KQkq'; c = order.split('').filter(f => (c + flag).includes(f)).join(''); }
    setFenPart(chess, onFenChange, 2, c || '-');
  }
  const castleBtn = (flag, label) => (
    <label key={flag} style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
      <input type="checkbox" checked={castling.includes(flag)} onChange={() => toggleCastle(flag)} style={{ accentColor: '#6366f1', width: 15, height: 15 }} />
      <span style={{ color: '#d1d5db', fontSize: 13 }}>{label}</span>
    </label>
  );
  return (
    <div>
      <div style={sectionLabel}>Castling Rights</div>
...
// ── En Passant (standalone) ──
export function EnPassantControl({ chess, onFenChange }) {
  ...
  return (
    <div>
      <div style={sectionLabel}>En Passant</div>
      <select
        value={ep}
        onChange={e => setFenPart(chess, onFenChange, 3, e.target.value)}
        style={{ width: '100%', background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 8, color: '#fff', padding: '8px 12px', fontSize: 13, cursor: 'pointer' }}
      >
